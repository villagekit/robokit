...
void test_ramp_calculations(void) {
  ...
  TEST_ASSERT_EQUAL_UINT32(25, stepper_ramp.max_acceleration_distance_in_steps);
  ...
  TEST_ASSERT_EQUAL_UINT32(25, stepper_ramp_movement.acceleration_distance_in_steps);
}

void test_movement(void) {
  auto current_step_index = step_index++;

  auto current_step_period_in_microsecs = stepper_ramp_movement.next();
  auto current_status = stepper_ramp_movement.current_status;
  auto steps_completed = stepper_ramp_movement.steps_completed;
  ...
  auto acceleration_distance = stepper_ramp_movement.acceleration_distance_in_steps;

  TEST_ASSERT_EQUAL_UINT32(current_step_index, steps_completed - 1);

  if (current_step_index == 0) {
    TEST_ASSERT_EQUAL(StepperRampMovement::Status::START, current_status);
    TEST_ASSERT_EQUAL_UINT32(acceleration_steps[0], current_step_period_in_microsecs);
  } else if (current_step_index <= acceleration_distance) {
    TEST_ASSERT_EQUAL(StepperRampMovement::Status::RAMP_UP, current_status);
    TEST_ASSERT_EQUAL_UINT32(acceleration_steps[current_step_index], current_step_period_in_microsecs);
  } else if (steps_total - current_step_index <= acceleration_distance) {
    TEST_ASSERT_EQUAL(StepperRampMovement::Status::RAMP_DOWN, current_status);
    auto deceleration_index = deceleration_steps.size() - (steps_total - current_step_index);
...
void test_end(void) {
  ...
  TEST_ASSERT_EQUAL(StepperRampMovement::Status::END, stepper_ramp_movement.current_status);
}
